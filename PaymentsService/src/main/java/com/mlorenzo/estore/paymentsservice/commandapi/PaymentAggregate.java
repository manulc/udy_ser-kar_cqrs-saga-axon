package com.mlorenzo.estore.paymentsservice.commandapi;
...
public class PaymentAggregate {
	...
	// Anotación para que este constructor sea un manejador de Commands del Aggregate(En este caso, Commands de tipo ProcessPaymentCommand).
	// En este caso, este manejador de Commands crea instancias del Aggregate de esta clase PaymentAggregate usando como identificador de cada instancia, mediante la anotación @AggregateIdentifier, el id de un pago.
	...
	public PaymentAggregate(ProcessPaymentCommand processPaymentCommand) {
		...
		if(processPaymentCommand.getPaymentId() == null || processPaymentCommand.getPaymentId().isBlank())
			throw new IllegalArgumentException("Payment Id is required and cannot be empty");
		...
		PaymentProcessedEvent paymentProcessedEvent = PaymentProcessedEvent.builder()
				...
				.build();
		// Con este método, un Aggregate aplica y publica un evento(En este caso, de tipo PaymentProcessedEvent). Ésto significa que primero se invocará dentro del Aggregate al método manejador de eventos de tipo @EventSourcingHandler asociado a ese evento(Dentro de un Aggregate sólo puede haber un método de este tipo asociado a un determinado tipo de evento)
	...
	// Anotación para que este método sea un manejador de eventos del Aggregate(En este caso, de tipo PaymentProcessedEvent)
